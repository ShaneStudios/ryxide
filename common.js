...
async function idbSet(storeName, value) {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
         ...
         transaction.oncomplete = () => resolve(true); // Ensure transaction completes
...
async function clearAllStores() {
     ...
     return new Promise((resolve, reject) => {
          if (storesToClear.length === 0) return resolve(true); // Nothing to clear
          ...
          storesToClear.forEach(storeName => {
               if (db.objectStoreNames.contains(storeName)) {
                    ...
                    request.onerror = (event) => console.error(`Error clearing store ${storeName}:`, event.target.error); // Log error but don't reject immediately
               } else {
                   console.warn(`Store ${storeName} not found during clear operation.`);
               }
          });
     });
}
...
async function saveAllProjectsList(list) { console.warn("saveAllProjectsList is deprecated with IDB structure. Save summaries individually."); return true; }
...
async function saveProjectToStorage(project) {
    ...
    try {
         await idbSet(PROJECT_DATA_STORE_NAME, project);
         await idbSet(PROJECTS_STORE_NAME, summary);
         return true;
    } catch (e) { alert(`Failed to save project: ${e}`); return false; }
}
async function deleteProjectFromStorage(projectId) {
    try {
        await idbRemove(PROJECT_DATA_STORE_NAME, projectId);
        await idbRemove(PROJECTS_STORE_NAME, projectId);
        return true;
    } catch (e) { alert(`Failed to delete project: ${e}`); return false; }
}
...
async function callGeminiApi(prompt, apiKey, history = []) {
     ...
     const systemInstruction = { role: "system", parts: [{ text: "You are RyxAI, a friendly, encouraging, and helpful coding assistant in the RyxIDE web editor. Format code blocks using Markdown." }] };
...
const starterContentByLanguage = {
     html: `<!DOCTYPE html>\n<html>\n<head><title>New</title></head>\n<body>\n\t\n</body>\n</html>`, css: `body {\n\t\n}`, javascript: `console.log("new JS");`, python: `import sys\nprint("Python")`, markdown: `# New File`,
     ruby: `puts "Ruby"`, csharp: `using System;\nclass P{static void Main(){Console.WriteLine("C#");}}`, java: `class Main{public static void main(String[]a){System.out.println("Java");}}`,
