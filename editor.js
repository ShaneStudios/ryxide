document.addEventListener('DOMContentLoaded', () => {
    ...
    const runtimeManager = {
        ...
        initializeTerminalAndVM: async () => {
            ...
            updateCredits();
            try {
                if (!window.Terminal || !window.FitAddon) { throw new Error("xterm.js or FitAddon not loaded globally."); }
                xtermInstance = new window.Terminal({ cursorBlink: true, fontSize: 13, fontFamily: 'Consolas, "Courier New", monospace', theme: getXtermTheme(currentSettings.theme), rows: 20, convertEol: true });                 xtermFitAddon = new window.FitAddon.FitAddon();
                ...
                if (typeof V86Starter === 'undefined') { throw new Error("V86Starter library not loaded globally."); }

                v86Emulator = new window.V86Starter({ wasm_path: V86_WASM_PATH, memory_size: 512 * 1024 * 1024, vga_memory_size: 8 * 1024 * 1024, cdrom: { url: V86_LINUX_IMAGE_URL, async: true }, autostart: true, disable_keyboard: true, disable_mouse: true, screen_container: null, });
        ...
        runPythonCode: async (code) => { if (!PYTHON_BACKEND_URL) { alert("Python backend URL is not configured."); updateStatus('Py Backend Error', 'error'); return; } updateStatus('Running Python (Backend)...', 'info', 0); showLoader(loaderOverlay, loaderText, "Executing Python..."); if(xtermInstance) {xtermInstance.writeln('\r\n\x1b[1;34m--- Sending Python to Backend ---\x1b[0m'); xtermInstance.focus();} else { outputConsole.textContent = 'Sending Python to Backend...\n---\n'; } try { const response = await fetch(PYTHON_BACKEND_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify({ code: code }) }); const result = await response.json(); const target = xtermInstance || { write: (t) => { outputConsole.textContent += t; }, writeln: (t) => { outputConsole.textContent += t + '\n'; } }; target.writeln('\r\n\x1b[1;34m--- Backend Response Start ---\x1b[0m'); if (result.stdout) target.write(result.stdout.replace(/\n/g, '\r\n')); if (result.stderr) target.write(`\r\n\x1b[1;31m--- STDERR ---\r\n${result.stderr.replace(/\n/g, '\r\n')}\x1b[0m`); if (result.exit_code === 0) { target.writeln(`\r\n\x1b[1;32m--- Python finished (Backend) [Exit Code: 0] ---\x1b[0m`); updateStatus('Python Finished', 'success'); } else { target.writeln(`\r\n\x1b[1;31m--- Python failed (Backend) [Exit Code: ${result.exit_code}] ---\x1b[0m`); updateStatus('Python Error', 'error'); } if (!response.ok) { target.writeln(`\r\n\x1b[1;31mBackend HTTP Error ${response.status}: ${result.error || response.statusText}\x1b[0m`); updateStatus('Python Backend Error', 'error'); } } catch (error) { const target = xtermInstance || outputConsole; target.textContent += `\r\n--- ERROR ---\r\nNetwork/Fetch Error: ${error.message}\x1b[0m`; updateStatus('Python Network Error', 'error', 5000); } finally { hideLoader(loaderOverlay); if(xtermInstance) xtermInstance.write('\r\n'); else outputConsole.scrollTop = outputConsole.scrollHeight; } },
    ...
    function updateRunButtonState() {
         ...
         const runnableInternalV86 = ['ruby', 'csharp', 'c', 'cpp', 'shell'].includes(lang);
         ...
         let canRun = runnableInternalClient || runnableBackendPython || (runnableInternalV86 && isV86Ready);
